...
module.exports = async function(deployer, network, accounts) {
  ...
  const getReceiptProofFuncResult = await getReceiptProof(txReceipt, block, null, [txReceipt])
  console.log('* getReceiptProofFuncResult: ', getReceiptProofFuncResult.parentNodes)
  ...
  const beforeBalance = await Meta.balanceOf('0xa121d7b3c4174414ab69741a9a27dd809de91407')
  console.log('* beforeBalance: ', beforeBalance)

  await RootChainManagerTestInstance.exit(encodeData).then(console.log)

  const afterBalance = await Meta.balanceOf('0xa121d7b3c4174414ab69741a9a27dd809de91407')
...
async function getReceiptProofFunc(receipt, block, receipts) {
  const receiptsTrie = new Trie()

  console.log('* [getReceiptProofFunc] receipts: ', receipts)

  for (let i = 0; i < receipts.length; i++) {
    const siblingReceipt = receipts[i]
    console.log('* [getReceiptProofFunc] siblingReceipt: ', siblingReceipt)
    // const path = siblingReceipt.transactionIndex
    const path = rlp.encode(siblingReceipt.transactionIndex)
    console.log('* [getReceiptProofFunc] path: ', path)
    const rawReceipt = getReceiptBytes(siblingReceipt)
    console.log('* [getReceiptProofFunc] rawReceipt: ', rawReceipt)
    await receiptsTrie.put(path, rawReceipt)
    console.log('* [getReceiptProofFunc] receiptsTrie1: ', receiptsTrie)
  }

  const {node, remaining, stack } = await receiptsTrie.findPath(rlp.encode(receipt.transactionIndex), false)
  console.log('* [getReceiptProofFunc] stack: ', stack.map((value) => value.raw()))

  const result = {
    blockHash: await toBuffer(receipt.blockHash),
    parentNodes: stack.map((value) => value.raw()),
    // root: getRawHeader(block).receiptTrie,
    path: await rlp.encode(receipt.transactionIndex)
  }
  console.log('* [getReceiptProofFunc] result: ', result)
